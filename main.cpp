...
float DEGTORAD = 0.017453f;  // переменная для перевода в радианы
...
public:
	...
	void update() // функция движения вперед
	{
		if (thrust)
		{
...
int main()
{
	...
	Text text, gameover; // работа с текстом
	...
	Animation sExplosion(t3, 0, 0, 256, 256, 48, 0.5); // создание анимации
	...
	player *p = new player(); // генерация игрока
	...
	while (app.isOpen()) // основная логика
	{
		...
		while (app.pollEvent(event))
		{
			...
			if (event.type == Event::KeyPressed) // для того, чтобы игрок не зажимал клавишу стрельбы
		...
		if (Keyboard::isKeyPressed(Keyboard::Space)) // начало игры после нажатия пробела
		{
			gamestarted = true;
		}
		...
		if (gamestarted)
		{
			...
			if (Keyboard::isKeyPressed(Keyboard::Right)) p->angle += 3; // повороты игрока
			...
			for (auto a : entities)
				for (auto b : entities)
				{
					if (a->name == "asteroid" && b->name == "bullet") //столкновение снаряда и астероида
						if (isCollide(a, b))
						{
							a->life = false;
							b->life = false;
							...
							for (int i = 0; i < 2; i++) // генерация 2-х осколков
							{
								if (a->R == 15) continue;
					...
					if (a->name == "player" && b->name == "asteroid") // столкновение игрока и астероида
						if (isCollide(a, b))
						{
							b->life = false;
							Entity *e = new Entity();
