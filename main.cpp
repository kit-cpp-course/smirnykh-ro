...
game::game()
{
	...
	Texture t1, t2, t3, t4, t5, t6, t7; // инициализация переменных для файлов

	fileloader fileloader(t1, t2, t3, t4, t5, t6, t7, shotBuffer, explBuffer, music); // загрузчик файлов

	Sound Shot(shotBuffer), Explosion(explBuffer); // инициализация звуков

	TextBox TBOX; // текстовая коробка
	Text GameOver = TBOX.returnGameOver(); // надпись конца игры
	Text text = TBOX.returnText(); // надпись счета
	...
	Animation sExplosion(t3, 0, 0, 256, 256, 48, 0.5); // анимация спрайтов
...
fileloader::fileloader(Texture &t1, Texture &t2, Texture &t3, Texture &t4, Texture &t5, Texture &t6, Texture &t7, SoundBuffer &shotBuffer, SoundBuffer &explBuffer, Music &music)
{
